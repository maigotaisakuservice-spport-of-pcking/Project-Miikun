...
def setup():
    ...
    print("Only required if you want to use GitHub Actions for external training.")
    gh_repo = get_input("GitHub Repo (user/repo)", "none")
    gh_pat = get_input("GitHub Personal Access Token", "none")
...
"""
    ...
    # 7. Initial AI Creation (Optional but recommended for All-in-One)
    print("\n--- Phase 2: Initial AI Creation ---")
    do_train = get_input("Run initial scraping and model training now? (Requires GPU/Time)", "n")
    if do_train.lower() == 'y':
        print("\n[!] Starting data scraping...")
        run_cmd(f"{install_dir}/venv/bin/python backend/scripts/scrape_educational_data.py")
        print("\n[!] Starting initial training (LoRA) - This will run until complete...")
        # Use --no-timeout for the initial setup to ensure a complete base model
        run_cmd(f"{install_dir}/venv/bin/python backend/scripts/train.py --no-timeout")
    ...
    print("\n--- GitHub Integration ---")
    print(f"Webhook Secret (for Actions/Webhooks): {webhook_secret}")
