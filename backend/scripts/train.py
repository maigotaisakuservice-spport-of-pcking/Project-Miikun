...
def deliberate_and_filter(dataset, subject):
    ...
    # Detect GPU for llama-cpp-python
    gpu_layers = -1 if torch.cuda.is_available() else 0
    evaluator = Llama(model_path=GGUF_PATH, n_gpu_layers=gpu_layers, n_ctx=2048, verbose=False)
...
def train_subject(subject, data_file):
    ...
    # Training Config with CPU/GPU dynamic switching
    if torch.cuda.is_available():
        ...
        print("CUDA not available. Attempting CPU-only training (Low memory mode).")
        # Load in 8-bit or half-precision to fit in 16GB RAM if possible
        model = AutoModelForCausalLM.from_pretrained(
            BASE_MODEL,
            device_map="cpu",
...
def train():
    ...
    # CUDA Check (Warning only, allowed to continue on CPU)
    if not torch.cuda.is_available():
        print("Warning: CUDA is not available. Training will proceed on CPU (Extremely slow).")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no-timeout", action="store_true", help="Disable global MAX_MINUTES timeout")
    args = parser.parse_args()

    if args.no_timeout:
        MAX_MINUTES = 999999 # Practically unlimited for initial setup
        print("Global timeout disabled. Training will run until all subjects are complete.")
