...
async def trigger_manual_train(authorization: str = Header(...)):
    ...
    try:
        # 1. Run full local training cycle
        # We run it in a background process to avoid blocking the API
        subprocess.Popen(
            [os.getenv("VENV_PYTHON", "python3"), "backend/scripts/weekly_train_local.py"],
